...
function sortCitiesArray(arr) {
  return arr
    .sort((c1, c2) => c1.city.localeCompare(c2.city))
    .sort((c1, c2) => c1.country.localeCompare(c2.country));
}
...
class Selector {
  ...
  collectSelector(type) {
    ...
    if (this.arr.length > 0 && this.arr[this.arr.length - 1] > typeIndex) {
      throw new Error(
        'Selector parts should be arranged in the following order: element, id, class, attribute, pseudo-class, pseudo-element'
      );
    }

    this.arr.push(typeIndex);
  }

  element(value) {
    this.collectSelector('element');
    this.selector += value;
    return this;
  }

  id(value) {
    this.collectSelector('id');
    this.selector += `#${value}`;
    return this;
  }

  class(value) {
    this.collectSelector('class');
    this.selector += `.${value}`;
    return this;
  }

  attr(value) {
    this.collectSelector('attr');
    this.selector += `[${value}]`;
    return this;
  }

  pseudoClass(value) {
    this.collectSelector('pseudoClass');
    this.selector += `:${value}`;
    return this;
  }

  pseudoElement(value) {
    this.collectSelector('pseudoElement');
    this.selector += `::${value}`;
    return this;
  }
